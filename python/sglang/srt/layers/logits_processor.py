...
class LogitsProcessorOutput:
    ...
    # When --beam-search-lm-head-special-token-ids is set during beam search, next_token_logits
    # has shape [#seq, num_candidates] instead of [#seq, vocab_size]. Column j
    # corresponds to beam_search_candidate_token_ids[j]. Consumers must map indices back via
    # beam_search_candidate_token_ids[indices] to recover real token IDs.
    beam_search_candidate_token_ids: Optional[torch.Tensor] = None
...
class LogitsProcessor(nn.Module):
    def __init__(
        ...
        # --beam-search-lm-head-special-token-ids is set, decode forwards only compute logits
        # for the specified vocab rows, reducing the matmul from [B, H] x [H, V]
        # to [B, H] x [H, K].
        # Caveats: CUDA graph keys do not distinguish candidate mode (beam /
        # non-beam reuse one graph → non-beam requests unsupported); full-vocab
        # logit_bias / *_penalty are unsupported; missing EOS in candidates
        # typically forces generation to max_tokens.
        self.beam_search_lm_head_special_token_ids = get_server_args().beam_search_lm_head_special_token_ids
    ...
    def forward(
        ...
        beam_search_candidate_token_ids = self._get_beam_search_candidate_token_ids(
        ...
        if not logits_metadata.extend_return_logprob:
            ...
            logits = self._get_logits(
                ...
                lm_head,
                logits_metadata,
                beam_search_candidate_token_ids=beam_search_candidate_token_ids,
            )
            sampled_logits = (
        ...
        logprobs_result, sampled_logits = self.input_logprob_processor.forward(
            ...
            get_logits_fn=functools.partial(
                self._get_logits, beam_search_candidate_token_ids=beam_search_candidate_token_ids
            ),
    ...
    def _get_logits(
        ...
        When ``beam_search_candidate_token_ids`` is set, only the corresponding rows of the
        ...
        logits = self._compute_lm_head(
            hidden_states, lm_head, embedding_bias, beam_search_candidate_token_ids
        )
        ...
        if beam_search_candidate_token_ids is not None:
    ...
    def _compute_lm_head(
        ...
        if beam_search_candidate_token_ids is not None:
            assert hasattr(
                lm_head, "weight"
            ), "beam_search_lm_head_special_token_ids requires lm_head.weight to exist"
            special_weight = torch.index_select(
                lm_head.weight, dim=0, index=beam_search_candidate_token_ids
            )
    ...
    def _get_beam_search_candidate_token_ids(
        self,
        device: torch.device,
        logits_metadata: LogitsMetadata,
        lm_head: VocabParallelEmbedding,
        ...
        When --beam-search-lm-head-special-token-ids is set, only these vocab rows are
        ...
        if not logits_metadata.is_beam_search:
            return None
        if self.beam_search_lm_head_special_token_ids is None:
            return None
        ...
        if self.do_tensor_parallel_all_gather:
            if not self._beam_special_token_restriction_warned:
                logger.warning(
                    "Ignore --beam-search-lm-head-special-token-ids when TP vocab is sharded. "
        ...
        if hasattr(lm_head, "set_lora") and hasattr(lm_head, "apply_lora"):
            if not self._beam_special_token_restriction_warned:
                logger.warning(
                    "Ignore --beam-search-lm-head-special-token-ids for LoRA-wrapped lm_head."
                )
                self._beam_special_token_restriction_warned = True
            return None
        if not hasattr(lm_head, "weight"):
            if not self._beam_special_token_restriction_warned:
                logger.warning(
                    "Ignore --beam-search-lm-head-special-token-ids for lm_head without weight."
                )
                self._beam_special_token_restriction_warned = True
            return None
        ...
        if token_ids is None:
            ids = self.beam_search_lm_head_special_token_ids
            if not ids:
                raise ValueError(
                    "--beam-search-lm-head-special-token-ids resolved to an empty list."
                )
            if hasattr(lm_head, "weight"):
                vocab_size = lm_head.weight.shape[0]
                if min(ids) < 0 or max(ids) >= vocab_size:
                    raise ValueError(
                        "Found out-of-range token ID in --beam-search-lm-head-special-token-ids "
