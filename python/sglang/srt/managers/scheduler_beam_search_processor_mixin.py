...
class SchedulerBeamSearchProcessorMixin:
    ...
    def process_beam_search_prefill_result(
        ...
        beam_search_candidate_token_ids = logits_output.beam_search_candidate_token_ids
    ...
    def process_beam_search_decode_result(
        ...
        # When --beam-search-lm-head-special-token-ids is set, the topk indices are positions
        # in the restricted candidate set, not real token IDs. Map them back so
        # downstream expansion operates on actual vocab IDs.
        beam_search_candidate_token_ids = result.logits_output.beam_search_candidate_token_ids
        if beam_search_candidate_token_ids is not None:
            beam_output_top_tokens = beam_search_candidate_token_ids[beam_output_top_tokens]
    ...
    def _process_beam_search_prefill_result_single_req(
        ...
        Args:
            ...
            logprobs: Log probabilities tensor for all tokens [vocab_size] or
                [num_candidates] when --beam-search-lm-head-special-token-ids is set.
            device: Device where tensors are located
            beam_search_candidate_token_ids: When the LM head is restricted to a subset of
        ...
        if beam_search_candidate_token_ids is not None:
            topk_indices = beam_search_candidate_token_ids[topk_indices]
