...
- [2. Feature Details](#2-feature-details)
  ...
  - [2.6 Beam Search LM Head Special Token IDs](#26-beam-search-lm-head-special-token-ids)
...
| `--beam-search-lm-head-special-token-ids` | str | `None` | Restrict beam-search LM head logits to specified token IDs; enables CUDA-graph / penalty / EOS limitations (see [2.6](#26-beam-search-lm-head-special-token-ids)) |
...
### 2.6 Beam Search LM Head Special Token IDs

`--beam-search-lm-head-special-token-ids` restricts beam-search decode LM head logits to a candidate token subset, shrinking the matmul from `[B, H] × [H, V]` to `[B, H] × [H, K]`. Typical for GenRec / SID-style closed candidate sets. **Must be used together with `--enable-beam-search`** (setting it alone raises an error); TP=1 only.
...
**Risks and limitations** (review before enabling):

1. **CUDA graph keys do not distinguish candidate mode**: the graph key does not separate restricted vs full-vocab LM head, so beam and non-beam reuse the same graph. After enabling this flag, the server **cannot serve non-beam-search requests**; treat the instance as beam-only.
2. **Full-vocab sampling features are unsupported**: `logit_bias`, `frequency_penalty`, `presence_penalty`, and `repetition_penalty` require logits over the full vocabulary and are unavailable with this flag.
3. **Candidate sets may omit EOS**: if EOS is not in the candidate tokens, sequences usually cannot stop early and **typically run until `max_tokens` / `max_new_tokens`**. The candidate set should cover all legal outputs; include EOS (and any business end tokens) explicitly if normal early stopping is required.
...
```bash
--beam-search-lm-head-special-token-ids 1,2,3,151643
--beam-search-lm-head-special-token-ids 151669:153206
--beam-search-lm-head-special-token-ids 151669:153206,151645
```
