...
| `--lm-head-special-token-ids` | str | `None` | Restrict beam-search LM head logits to specified token IDs; enables CUDA-graph / penalty / EOS limitations (see [2.6](#26-lm-head-special-token-ids)) |
...
`--lm-head-special-token-ids` restricts beam-search decode LM head logits to a candidate token subset, shrinking the matmul from `[B, H] × [H, V]` to `[B, H] × [H, K]`. Typical for GenRec / SID-style closed candidate sets. **Must be used together with `--enable-beam-search`** (setting it alone raises an error); TP=1 only.
